...
- CLI npm packaging is fully self-contained: `src/cli/package.json` sets `"bundleDependencies": true`, and `prepack` (`src/cli/scripts/prepare-bundle.mjs`) copies the built `@commandgarden/*` packages AND copies the entire third-party production dep closure from the monorepo's root `node_modules/` into `cli/node_modules/` (exact, lockfile-pinned versions via `npm ls --omit=dev --all --parseable` — no pack-time `npm install`, so bundled versions never drift from what was tested). This is required because npm treats a bundleDependency's own deps as `inBundle` and will NOT fetch them on a global `npm i -g` — a partial bundle leaves empty `node_modules/zod` (etc.) dirs and the daemon dies with `ERR_MODULE_NOT_FOUND` (only global installs break; local installs mask it via hoisting). Do not revert to a partial/array `bundleDependencies`. The script refuses to bundle any dep declaring `os`/`cpu` constraints (a resolved on-disk tree is never re-resolved at install time). Keep build-only deps (React, Vite, etc.) in `devDependencies` so they stay out of the bundled prod closure.
